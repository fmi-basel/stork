...
def make_coRandman(
    ...
    """Generates event based generalized tempo randman classification dataset.
    This is an extension of the `make_tempo_randman`.

    This dataset creates a concatenated randman task, where two randman samples are separated by `T_delay`.
    The label is then the index of the sorted concatenated labels of the original samples.
    ...
    Args:
        ...
        classification: Whether to generate a classification (default) or regression dataset
        seed: The random seed (default: None)
        T_delay: time between the two samples
    ...
    for d1, l1 in zip(data, labels):
        times1, units1 = d1
        for d2, l2 in zip(data, labels):
            times2, units2 = d2
            units = torch.cat([units1, units2])
            times = torch.cat([times1, times2 + T_delay])
            new_data.append((times, units))
            new_labels.append(int(str(l1) + str(l2)))

    labels = np.unique(new_labels)
    transform_labels = {l: i for i, l in enumerate(labels)}

    transformed_new_labels = []
    for l in new_labels:
