...
class AbstractLayer():
    ...
    def add_connection(self, connection, recurrent='detect', inhibitory=False):
        ...
        assert connection.dst in self.neurons, "Target neuron group is not in this Layer"
...
class Layer(AbstractLayer):
    ...
    def __init__(self, name, model, size, input_group, recurrent=True, regs=None, w_regs=None,
        ...
        # Make neuron group
        nodes = neuron_class(size, name=self.name, regularizers=regs, 
                             **neuron_kwargs)
        ...
        if recurrent:
            con = connection_class(nodes, nodes, regularizers=w_regs, 
                                   **connection_kwargs)
            self.add_connection(con)
...
class ConvLayer(AbstractLayer):
    ...
    def __init__(self, name, model, input_group, 
                 kernel_size, stride, padding=0, nb_filters=16, shape='auto', 
                 recurrent=True, regs=None, w_regs=None, 
                 connection_class=connections.ConvConnection, neuron_class=nodes.LIFGroup, 
                 neuron_kwargs={}, connection_kwargs={}, recurrent_connection_kwargs={}) -> None:
        ...
        if shape == 'auto':
            assert isinstance(nb_filters, int), "Must provide nb_filters to calculate ConvLayer shape"
            
            shape = utils.convlayer_size(nb_inputs = input_group.shape[1:], 
            ...
            assert isinstance(shape, tuple), "`shape` must be 'auto' or a tuple of integers"
                            
        # Make neuron group
        nodes = neuron_class(shape, name=self.name, regularizers=regs, 
                             **neuron_kwargs)
        self.add_neurons(nodes)
        ...
        if recurrent:
            ...
            con = connection_class(nodes, nodes, regularizers=w_regs,
                                   kernel_size=rec_kernel_size, stride=rec_stride, padding=rec_padding,
