...
class AdaptLearnLIFGroup(LIFGroup):
    ...
    def configure(self, batch_size, nb_steps, time_step, device, dtype):
        ...
        adapt_b = torch.rand(self.shape, device=device,
    ...
    def reset_state(self, batch_size=None):
        ...
        self.dcy_ada = torch.exp(-self.dt_ /
                                 (torch.sigmoid(self.ada_dcy_param) * self.tau_ada))
    ...
    def forward(self):
        ...
        new_mem = (self.dcy_mem * self.mem + self.scl_mem *
                   (self.syn - self.adapt_b * self.ada)) * (1.0 - rst)
