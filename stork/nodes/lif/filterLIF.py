...
class FilterLIFGroup(CellGroup):
    def __init__(
        ...
        **kwargs
        ...
        assert self.nb_units % self.nb_groups == 0, "Nb_units must be divisible by nb_groups"
        self.nb_units_per_group = self.nb_units // self.nb_groups
        self.group_membership = torch.tensor(list(range(self.nb_groups)) * self.nb_units_per_group)
    ...
    def reset_state(self, batch_size=None):
        ...
        ).reshape(shape=(self.nb_units, self.nb_filters)).to(self.device)
    ...
    def forward(self):
        ...
        # synaptic dynamics
        # Update filters (z=f)
        new_filt = torch.einsum("bnf,fz->bnz", self.filt, self.A)
    ...
    def get_filterbanks_by_group(self, sum_over_filters=True, T=None,  A=None, filt_param=None):
        """
    ...
    def get_membrane_kernel_by_group(self, time_step=None):
        ...
        syn_currents = self.get_filterbanks_by_group(sum_over_filters=True,
                                                    T=T, 
                                                    A = A, 
                                                    filt_param = filt_param)
        
        mem_by_group = lif_membrane_dynamics(syn_currents, 
                                             self.tau_mem, 
                                             dt=time_step)
