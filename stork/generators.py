...
class StandardGenerator(DataGenerator):
    ...
    def __call__(self, dataset, shuffle=True):
        return torch.utils.data.DataLoader(dataset, batch_size=self.batch_size, shuffle=shuffle, num_workers=self.nb_workers, persistent_workers=self.persistent_workers)
...
class SparseGenerator(DataGenerator):
    ...
    def get_sparse_collate(self):
        ...
        def collate_function(data_labels):
            ...
            neuron_indices = torch.FloatTensor(
                np.concatenate([ind for _, ind in data]))
            indices = torch.LongTensor(
                np.stack([batch_indices, neuron_indices],  axis=0))
            values = torch.FloatTensor(np.concatenate(
                [np.ones(len(x[0])) for x in data]))
            return [(indices[:, time_indices == t], values[time_indices == t]) for t in range(self.nb_steps)], torch.stack(labels, dim=0)

        return collate_function

    def __call__(self, dataset, shuffle=True):
        return torch.utils.data.DataLoader(dataset, batch_size=self.batch_size, shuffle=shuffle, num_workers=self.nb_workers, collate_fn=self.get_sparse_collate())
