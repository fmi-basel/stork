...
def add_xscalebar(ax, length, label=None, pos=(0.0, -0.1), off=(0.0, -0.07), **kwargs):
    add_scalebar(ax, label=label, extent=(length, 0.0), pos=pos, off=off,
                 verticalalignment="top", horizontalalignment="center", **kwargs)


def add_yscalebar(ax, length, label=None, pos=(-0.1, 0.0), off=(-0.07, 0.0), **kwargs):
    add_scalebar(ax, label=label, extent=(0.0, length), pos=pos, off=off,
                 verticalalignment="center", horizontalalignment="left", rotation=90, **kwargs)


def dense2scatter_plot(ax, dense, point_size=5, alpha=1.0, marker=".", time_step=1e-3, jitter=None, **kwargs):
    ras = datasets.dense2ras(dense, time_step)
    if len(ras):
        noise = np.zeros(ras[:, 0].shape)
        if jitter is not None:
            noise = jitter*np.random.randn(*ras[:, 0].shape)
        ax.scatter(ras[:, 0]+noise, ras[:, 1], s=point_size,
                   alpha=alpha, marker=marker, **kwargs)
...
def plot_activity_snapshot(model, data=None, labels=None, pred=None, nb_samples=5, plot_groups=None, marker=".",
    ...
    for i in range(nb_samples):
        ...
        if show_predictions:
            ax.text(0.5, 0.8, "Pred: %i" %
                    pred[k], color="black", transform=ax.transAxes, fontsize=8)
