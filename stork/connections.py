...
class SuperConnection(BaseConnection):
    ...
    def get_filterbanks(self, T=1):
        ...
        w = self.op.weight
        weighted_filters = w.view(1, *w.shape) * output.view(
            timesteps + 1, 1, self.src_shape * self.nb_filters
        )

        weighted_filters = weighted_filters.reshape(
            timesteps + 1, w.shape[0], self.src_shape, self.nb_filters
        )
...
class SuperConvConnection(SuperConnection):
    ...
    def get_filterbanks(self, T=1):
        ...
        w = self.op.weight
        weighted_filters = w.view(1, *w.shape) * output.view(
            timesteps + 1, 1, self.src_shape * self.nb_filters, 1
        )
